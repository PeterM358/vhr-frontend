...
export default function NetworkClaimsListScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await listNetworkClaims(token, org.id, 'mine');
  ...
  return (
    <ScreenBackground>
      <AppNavigationBar title={t('network.claims.myClaims')} onBack={() => navigation.goBack()} />
      <ScrollView contentContainerStyle={styles.content}>
        <Button mode="contained" onPress={() => navigation.navigate('NetworkClaimCreate')}>
          {t('network.claims.create')}
        </Button>
