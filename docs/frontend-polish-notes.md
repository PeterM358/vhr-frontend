...
**What to build next:** `/Users/client/vhr/docs/implementation-priority.md`  
**Geo provider discovery (API + UX):** `/Users/client/vhr/docs/backend-service-provider-discovery.md`  
**Workshop registration finalize:** `/Users/client/vhr/docs/client-workshop-registration-flow.md`
...
- **Vehicle dashboard → read-only Vehicle specs:** tapping the dark hero identity row (plate, make/model, registration lines, km summary) or **View specs** opens **`VehicleSpecs`** (**Vehicle specs**), a read-only dashboard of identity, technical/maintenance optional groups (context-aware, non-empty fields only), and mileage evidence. **Edit technical details** remains a separate action and still navigates to `EditVehicleDetailsScreen`; **Update kilometers** stays the only km edit entry (modal; PATCH `kilometers` only). No duplicate inline **Update** next to the km line.
...
- **Vehicle registration (single date):** **`first_registration_date`** is the only registration date field — no separate model-year textbox. **`Vehicle.year`** is not edited in the app; lists use **`registration_year`** from the API when present, else legacy **`year`**. Hero shows **`ISO2 • Registered DD.MM.YYYY`** under make/model when country and/or date exist (legacy **~YYYY** when only old year exists); **no year pill badge**. Create/edit use **`VehicleRegistrationIdentityBlock`** (first registration date picker + country picker, ISO2).
- **Registration country:** same **`getCountries()`** list as profiles; picker stores **ISO2**. Technical collapsible sections **do not** repeat registration date/country (moved out of optional technical groups).
- **Duplicate vehicle-details card removed:** the extra **More vehicle details** card is removed from `VehicleDetailScreen`; optional fields are summarized on the dashboard and fully listed read-only on **`VehicleSpecs`**; **Edit technical details** / **Update kilometers** remain the edit entry points.
- **Normal edit locks core identity and VIN:** `EditVehicleDetailsScreen` shows plate, vehicle type, brand/model, and **VIN** read-only; **Registration** (first registration date + country) is editable in a separate card and PATCHes **`first_registration_date`** / **`registration_country`**. VIN is **not** editable in normal edit (future correction flow). Save payloads exclude locked identity and kilometers; technical optional groups exclude registration fields.
...
- **Mileage evidence UI:** `MileageEvidenceCard` renders backend `mileage_confidence` (category, summary, ✓/⚠ factors, rollback warnings). Used on **Edit technical details** and **Vehicle specs**. Raw scores are not shown.
- **Mileage evidence read-only in normal edit:** `EditVehicleDetailsScreen` no longer exposes editable `odometer_verified`
  or `odometer_source`; normal edit shows the confidence card and excludes those fields
- **Odometer photo (optional):** **Log service record** → **Add odometer photo (optional)** uploads `vehicle_photo` with title `Odometer photo` (evidence only; no OCR). Helper copy explains lower km is allowed; warnings affect confidence only.
...
- **Owner-logged trust loop (RepairDetail):** for `source=owner_logged` records, provider trust card now separates
  **Workshop attributed** vs **Confirmation requested** vs **Confirmed by service center** vs **Did not confirm**.
  Owner can tap **Request confirmation** (only when linked `shop_profile` exists); attached shop users can **Confirm**
  or **Reject** pending requests. Manual/unlisted provider shows no platform confirmation available.
- **Evidence chips on service records:** owner-logged detail now surfaces lightweight evidence chips (photos, receipt,
  odometer-photo hint where tagged) without conflating these with service-center confirmation.
- **Post-save confirm CTA:** after logging a service record with an authorized center selected, owners get optional CTA
  **Ask service center to confirm this record** (no auto-request unless explicitly chosen).
...
- **Open service request plate privacy:** in the hero, **license plate** is shown to the **owner** and to shops with **booked** access or **owner-granted authorization** (`Vehicle.shared_with_shops`). Other shops on **open** requests see **“Plate hidden until booking”**; make/model stay visible. Backend masks `vehicle_license_plate` in repair API when not trusted ([`repairs/shop_visibility.py`](/Users/client/vhr/repairs/shop_visibility.py)).
- **Authorized service centers on vehicle detail:** list comes from **`shared_with_shops`** on `GET /api/vehicles/:id/` (vehicle detail serializer). Cards are **tappable** (shop profile) with **Remove access**; **Manage service center access** opens **`ManageVehicleServiceCentersScreen`** (authorize/revoke only—**not** tied to logging a service record). Map flow uses `returnTo: VehicleDetail` or `ManageVehicleServiceCenters`. Revoking access removes ongoing vehicle visibility; shops **retain** repairs/offers they already had ([`repairs/views.py`](/Users/client/vhr/repairs/views.py) `_user_can_access_repair`).
...
- **Final service type in repair management:** `RepairDetailScreen` ongoing shop view now exposes a **Final service type** selector (prefers `final_repair_type`, falls back to requested `repair_type`) and sends `final_repair_type` on save/finalize with helper context for service history, reminders, and statistics. **2026-08-07:** Finalize reuses the same **`OperationTypePickerSheet`** + **`buildOperationTypePickerOptions`** path as Add operation (category groups + search), filtered by shop business category / `supported_vehicle_types` / `available_repairs` and by the job’s `vehicle_type_id` when present (car RO hides bike-only types). Does **not** fail-open to the full `/types/` catalog when the shop profile is known. Two-step category drill-down still deferred.
...
- **Edit technical details — registration:** summary rows live under **Vehicle identity** (first registration + reg. country). Editor is **collapsed by default**; expands via **Add/Edit registration**. Native uses **compact rows + bottom-sheet pickers** (no full-height spinners). First registration date **locks after first save**; **country stays editable** (e.g. re-registration abroad). Still manual entry — no VIN/plate autofill yet. **Create vehicle** keeps the same registration block (compact on native).
- **VehicleDetail collapsible sections:** non-critical sections are collapsible (**Reminders & obligations**, service history, authorized centers, documents when files exist). **Quick actions** card removed (duplicated FAB, vehicle info tiles, service history). **Documents & photos** hidden until at least one file exists on the vehicle.
...
- **Add activity wording:** vehicle actions use **Add activity** with separate intents (see below). Backend now distinguishes **`Repair.source`**: **`marketplace_request`** (Request Service, `open`) vs **`owner_logged`** (past work, `done`) vs shop **`service_center_direct`** (future walk-in UI).
- **Add activity split (implemented):** From `VehicleDetailScreen`, **Add activity** → **Request service** (`CreateRepairScreen`, `mode=request`, *Send request*, `source=marketplace_request`, `status=open`). **Add service record** (`LogServiceRecordScreen`) is **completed maintenance/repair work only** (subtitle: *Use this for completed maintenance or repair work.*). **Add obligation / payment** opens **`AddObligationPaymentScreen`** (subtitle: *Use this for insurance, vignette, road tax, and inspection dates/payments.*) — separate from service records; still use **Reminders & obligations** rows for quick edits. **Upload receipt/document** stays out of this menu.
- **Service record type picker:** `filterServiceRecordRepairTypes` (`src/utils/serviceRecordRepairTypes.js`) drops obligation-shaped types: whole **`insurance-documents`** category, name matches for vignette / road tax, and **`inspections-legal`** except **`technical-inspection`** (performed inspection as work). Insurance renewal, vignette, and road tax belong under **Add Obligation / Payment**, not repairs.
- **Type-aware service record UI:** `classifyServiceRecordFormVariant` drives fields — **Oil** (oil change / name match): km required, optional next due km/date (PATCH `oil_service` reminder after create if set; backend still applies default interval on `Repair` save). **Technical inspection** (slug `technical-inspection`): **valid until / next inspection due** required (PATCH `technical_inspection` reminder); **total paid** only (no labor/parts split in UI); optional km hidden. **Brake**: labor/parts/total, optional **recommended next check km** (PATCH `brake_check` if set). **Generic**: km optional, notes, labor/parts/total, manual shop, self repair.
- **Add Service Record screen (`LogServiceRecordScreen`):** layout uses **`FloatingCard`** sections (**Service**, **Service date & mileage**, **Costs**, **Service provider**, **Notes & evidence**) with a sticky **Save service record** bar. **Completed date** uses **`ServiceRecordDatePicker`** (native pickers + web `type="date"`): shown as **DD.MM.YYYY**, stored/sent as **ISO `YYYY-MM-DD`**, default **today**, **today or past only** (no future completed date). **Next oil due date** uses the same picker component, **optional**, **future dates allowed**. **Valid until / next inspection** uses the picker for calendar entry.
- **Service record kilometers vs vehicle odometer:** old records may use **lower** kilometers than the vehicle profile; the backend **never decreases** vehicle current km — it only **increases** when the logged value is higher. The form shows helper copy explaining this.
- **Log Service Record — service provider (`LogServiceRecordScreen`):** **Self repair** toggle; **Service center** picker merges **authorized** (`shared_with_shops`, `· authorized`) and **unconfirmed** workshops from this vehicle’s past owner-logged repairs (`knownVehicleWorkshops.js`). **Add workshop (map + phone)** opens **`AddManualServiceCenter`** (E.164 phone, map pin, city/country DB pickers). When list &gt; 8: search, city filter, **Near me** (40 km, workshops with coordinates). Payload: `self_repair`, `shop_profile`, or `manual_service_center_*`. **Service type** on the log form + **vehicle type** on the vehicle implicitly classify the workshop for map filters (backend candidate upsert — see `client-workshop-registration-flow.md`). **Next:** wire **`manual_service_center_notes`** on Add workshop; geo API + **owner-reported pins** on `ShopMapScreen` (same vehicle/repair type chips as verified shops). **RepairDetail** shows provider via `formatServiceRecordProvider` + `formatOwnerLoggedTrustLabel` (`src/utils/serviceRecordProvider.js`).
- **Service record attachments (`LogServiceRecordScreen`):** optional **Add receipt / invoice** and **Add photo** in **Notes & evidence**; files queue locally and upload **after** the record is created via **`POST /api/vehicles/<id>/repairs/<repair_id>/documents/`** (`receipt` / `repair_invoice` / `vehicle_photo`). Failed uploads **do not roll back** the saved record — alert: *Service record saved, but some documents failed to upload.* Helpers: `src/api/documents.js`, `src/utils/pickDocumentFile.js`.
- **Obligation document upload (`AddObligationPaymentScreen`):** optional **Add policy or receipt** uploads to **`POST /api/vehicles/<id>/documents/`** with `document_type` mapped from obligation type (`insurance_policy`, `technical_inspection`, `vignette`, `road_tax`), plus **`valid_until`**, **`total_amount_minor`** when amount entered, **`paid_date`**, and **notes**. Reminder save succeeds even if document upload fails.
- **Vehicle documents archive (`VehicleDetailScreen`):** **Documents & photos** loads **`GET /api/vehicles/<id>/documents/`** (owner only) and groups rows into **Invoices / receipts**, **Insurance / inspection / vignette**, **Vehicle photos**, **Other** with counts. OCR / auto parts extraction remain future (manual amount/date for now).
- **Owner-logged repair money:** `resolveOwnerLoggedRepairMoney` — if only **`total_price`** is set and labor/parts empty → **`labor_price=0`**, **`parts_price=total`** (major units, same as existing repair API). **`VehicleExpense`** on the obligation screen uses **`amount_minor`** (BGN×100) via `createVehicleExpense`; consider a dedicated money helper / currency exponent TODO if multi-currency or non–2-decimal amounts matter.
- **Obligations screen:** PATCH `VehicleReminder` (`due_date`, `source_note`); optional POST **`/api/vehicles/<id>/expenses/`** when amount is filled (`createVehicleExpense` in `src/api/vehicles.js`). Document placeholder only; failed expense shows an alert, reminder still saved.
- **RepairDetail / history:** unchanged summary: `source=owner_logged` + `done` → service record framing; **VehicleDetail** history line still prefers manual shop / self repair.
...
- **Log service record:** owner past work is a **`Repair`** with **`source=owner_logged`** and **`status=done`** (API-enforced); does not trigger open-request shop notifications. The **Add Service Record** flow POSTs `final_kilometers`, `completed_at`, types, `self_repair`, manual service center fields (when applicable), and `evidence_level` as documented above. **Manual unlisted center:** dedicated **`AddManualServiceCenter`** screen (stack: record → add center → map picker); **Use for this record** attaches draft to this record only (not the authorized-centers picker). Requires **phone or email** plus one location anchor; **`MapLocationPicker`** uses a single dark chrome bar (no duplicate stack back). **Costs:** labor/parts auto-sum into total (total remains editable).
...
- **Progressive vehicle profile (create & detail):** `CreateVehicleScreen` and `EditVehicleDetailsScreen` tuck extended backend fields into **collapsed** sections: **Technical**, **Maintenance specifications** (helper: not service reminders), **Bike / e-bike / suspension**, **EV / hybrid**, **Trailer / towed equipment**, **Fleet / business**, **Mileage evidence**. On `VehicleDetailScreen`, **read-only** optional section cards show **only when at least one value exists**; **VehicleSpecs** is the full read-only specs view from the hero / **View specs**. Owners use **Edit technical details** for optional fields and **Update kilometers** for odometer. **VIN** is shown read-only in technical edit and in detail summary; normal edit does not change VIN. Numeric optional fields omit empty input or validate before save; `getVehicleTypes()` uses `/api/vehicles/types/` when available.
...
## Push notifications (FCM) — frontend contract

Backend reference: [`/Users/client/vhr/docs/backend-notifications-and-push.md`](/Users/client/vhr/docs/backend-notifications-and-push.md).

- **Register after login / token restore:** [`syncPushDeviceToken`](../src/notifications/pushDeviceSync.js) requests native permission (when available), reads FCM token, and **POST**s to **`/api/notifications/device-tokens/`** with `{ token, platform, app_version, app_build }`. Wired from **`login`**, **`register`**, **`googleLogin`**, and **`AuthManager`** on cold start.
- **Token refresh:** `messaging().onTokenRefresh` re-POSTs the same endpoint (`attachPushTokenRefreshListener`).
- **Logout:** [`deactivatePushDeviceToken`](../src/notifications/pushDeviceSync.js) **POST**s **`/api/notifications/device-tokens/deactivate/`** with the last stored token, then clears auth storage.
- **Safe on web / simulator:** `@react-native-firebase/messaging` is skipped on web; failures are caught — app continues without push.
- **Background handler:** registered in [`index.js`](../index.js) via `registerBackgroundMessageHandler()` before React mounts (native only). Foreground alerts stay in `registerFirebaseListeners`.
- **WebSocket unchanged:** in-app realtime still uses [`WebSocketManager`](../src/context/WebSocketManager.js).
- **Secrets:** never ship Firebase service account or Google OAuth client secret in the app.
