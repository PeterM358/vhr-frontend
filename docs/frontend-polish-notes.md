...
- **Log Service Record — service provider (`LogServiceRecordScreen`):** three mutually exclusive options — **Self repair** (toggle), **Authorized service center** (picker from `vehicle.shared_with_shops` on vehicle detail payload), **Add service center manually** (name / phone / email / address; address required if any contact field filled). Default: no provider. Payload: `self_repair`, `shop_profile`, or `manual_service_center_*` only for the active mode. Copy clarifies authorized selection is **not** center confirmation yet. **VehicleDetail** link: *Add service center when logging a service record* → `LogServiceRecord` with `providerMode: 'manual'`. **RepairDetail** shows **Service provider** via `formatServiceRecordProvider` and a simple trust hint via `formatOwnerLoggedTrustLabel` (`src/utils/serviceRecordProvider.js`). Map pick for manual address remains disabled placeholder.
